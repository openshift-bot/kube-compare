...
FROM registry.ci.openshift.org/ocp/builder:rhel-8-golang-1.26-openshift-4.23 AS builder-rhel-8
ARG SRC_DIR
WORKDIR ${SRC_DIR}
COPY . .
RUN make cross-build --warn-undefined-variables

FROM registry.ci.openshift.org/ocp/builder:rhel-9-golang-1.26-openshift-4.23 AS builder-rhel-9
ARG SRC_DIR
WORKDIR ${SRC_DIR}
COPY . .
RUN make cross-build --warn-undefined-variables

FROM registry.ci.openshift.org/ocp/4.23:cli AS final-builder
ARG SRC_DIR
